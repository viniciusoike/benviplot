...
load (requires the `systemfonts` package). By default, `theme_benvi()` uses
the system sans-serif font so the package works on any rendering device without
warnings.
...
To make this permanent, add that line to your `~/.Rprofile`. For the best
rendering quality, also install the `ragg` package:
...
MIT License. See [LICENSE.md](https://github.com/viniciusoike/benviplot/blob/master/LICENSE.md) for details.
