...
The `benviplot` package is the new extension of the previous `quinto_andar_` functions. Theses are `ggplot2` wrappers and helper functions to create standardized plots that follow the colors of QuintoAndar Group. The umbrella company name Benvi was announced officially in the second semester of 2022 and provides a much better name for the functions.
...
Since `benviplot` uses `ggplot2` functions it is always advisable to load both packages. 
