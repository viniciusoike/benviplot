...
- **Benvi Brand Palettes**: Benvi-focused colors (qual_benvi, div_blue_muted, div_purple_muted)
...
A qualitative palette combining Benvi's signature colors - perfect as the default for multi-group visualizations:

```{r qual-benvi}
benvi_palette("qual_benvi")
```

**When to use**: As the new default for categorical data visualization, especially in Benvi-branded materials.

**Example**:

```{r qual-benvi-example}
ggplot(iqaiw_total, aes(x = date, y = index, color = name_muni)) +
  geom_line(linewidth = 1) +
...
Diverging/muted palettes for subtle comparisons or neutral visualizations:
...
**When to use**:
- Diverging data around a neutral midpoint
- Subtle, professional visualizations
- When you need softer color contrasts
