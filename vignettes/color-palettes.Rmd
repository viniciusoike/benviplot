...
# Display all Set palettes
set_names <- paste0("Set", 0:7)

for (pal_name in set_names) {
...
ggplot(mtcars, aes(x = wt, y = mpg, color = as.factor(carb))) +
  geom_point(size = 3) +
  scale_color_benvi_d(pal_name = "Qual3", name = "Carburetors") +
  labs(
...
ggplot(heatmap_data, aes(x = x, y = y, fill = value)) +
  geom_tile() +
  scale_fill_benvi_c(pal_name = "Seq3", name = "Value") +
...
```{r index-palettes}
# Blue index
benvi_palette("index_blue")

# Purple index
benvi_palette("index_prpl")
```
...
ggplot(index_data, aes(x = date, y = index)) +
  geom_line(color = benvi_palette("index_blue")[7], linewidth = 1) +
  geom_area(fill = benvi_palette("index_blue")[3], alpha = 0.3) +
  labs(
...
# Reversed
benvi_palette("Seq5", direction = -1)
```
...
ggplot(heatmap_data, aes(x = x, y = y, fill = value)) +
  geom_tile() +
  scale_fill_benvi_c(pal_name = "Seq5", direction = -1, name = "Value") +
...
my_color <- benvi_palette("Qual5")[3]
my_color
...
benvi_palette("Seq2", n = 15, type = "continuous")
```
...
custom_colors <- c(
  benvi_palette("Set3")[1],
  benvi_palette("Qual5")[4],
  benvi_palette("rio_qual")[2],
  benvi_palette("Seq7")[8]
)
...
|-----------|-------------|----------|-------------|
| 2-3 categories | Basic | Basic1, Basic2, Basic3 | 3 |
| 2-4 categories | Set | Set0-Set7 | 4 |
| 5-8 categories | Qualitative | Qual1-Qual9 | 8 |
| Continuous data | Sequential | Seq0-Seq7 | 9 (interpolated) |
| Geographic (cities) | City-specific | spo_*, rio_*, bhe_* | 4-8 |
| Single metric | Index | index_blue, index_prpl | 9 |
...
# Display Set palettes
show_palette_grid(paste0("Set", 0:7), "Set Palettes")
```

```{r gallery-qual, fig.height=12}
# Display Qualitative palettes
show_palette_grid(paste0("Qual", 1:9), "Qualitative Palettes")
```

```{r gallery-seq, fig.height=12}
# Display Sequential palettes
show_palette_grid(paste0("Seq", 0:7), "Sequential Palettes")
```
...
- [Viz Palette](https://projects.susielu.com/viz-palette) - Test color palettes
